...
public class RightViewImpl<T> implements TreeView<T> {
    ...
    public void view(NodeWithLevel<T> root) {
        ...
        while (queue.size() != 0) {
            ...
            List<NodeWithLevel<T>> children = node.getChildren();
            for (int childIndex = node.getChildren().size() - 1; childIndex >= 0; childIndex--) {
                NodeWithLevel<T> child = children.get(childIndex);
